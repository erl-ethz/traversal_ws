...
RPGRotorsInterface::RPGRotorsInterface(const ros::NodeHandle& nh,
    ...
    : nh_(nh),
      ...
      integral_term_(),
      derivative_term_(),
      q_e_previous_(1,0,0,0)
{
  loadParameters();

  rotors_desired_motor_speed_pub_ =
      nh_.advertise<mav_msgs::Actuators>("command/motor_speed", 1);
  low_level_torque_and_thrust_pub_ = 
      nh_.advertise<quadrotor_msgs::ControlCommand>("low_level_control_command", 1);
  pid_terms_pub_ = 
      nh_.advertise<quadrotor_msgs::PID>("pid_low_level", 1);
  ...
  interaction_sub_ = nh_.subscribe("interaction", 1,
      &RPGRotorsInterface::InteractionCallback, this);
...
void RPGRotorsInterface::lowLevelControlLoop(const ros::TimerEvent& time) {
  ...
  if (!interface_armed_ || !control_command_.armed) {
    ...
    if (control_command_.control_mode == control_command_.ATTITUDE) {
      ...
      quadrotor_msgs::ControlCommand torque_and_thrust_msg = toRosMsg(torques_and_thrust);
      torque_and_thrust_msg.header.stamp = ros::Time::now();
      torque_and_thrust_msg.header.seq++;
      low_level_torque_and_thrust_pub_.publish(torque_and_thrust_msg);
      ...
      desired_motor_speed = mixer(torques_and_thrust);
    // } else if(control_command_.control_mode ==
    //           control_command_.TORQUES) {
    //   TorquesAndThrust torques_and_thrust;
    //   torques_and_thrust.body_torques = quadrotor_common::geometryToEigen(control_command_.torque);
    //   torques_and_thrust.collective_thrust = control_command_.collective_thrust;
...
quadrotor_common::ControlCommand RPGRotorsInterface::attitudeControl(
  ...
  double dt = 1/low_level_control_frequency_;
  
  updateIntegralTerm(q_e, dt);
  updateDerivativeTerm(q_e_previous_, q_e, dt);
  ...
  if (q_e.w() >= 0) {
    body_rate_command.bodyrates.x() =
        attitude_cmd.bodyrates.x() + 
        2.0 * roll_pitch_cont_gain_ * q_e.x() + 
        integral_gain_ * integral_term_.x + 
        derivative_gain_ * derivative_term_.x;
    body_rate_command.bodyrates.y() =
        attitude_cmd.bodyrates.y() + 
        2.0 * roll_pitch_cont_gain_ * q_e.y() + 
        integral_gain_ * integral_term_.y + 
        derivative_gain_ * derivative_term_.y;
    body_rate_command.bodyrates.z() =
        attitude_cmd.bodyrates.z() + 
        2.0 * roll_pitch_cont_gain_ * q_e.z() + 
        integral_gain_ * integral_term_.z + 
        derivative_gain_ * derivative_term_.z;
  } else {
    body_rate_command.bodyrates.x() =
        attitude_cmd.bodyrates.x() - 
        2.0 * roll_pitch_cont_gain_ * q_e.x() - 
  ...
  // t_termination_attitude_control_ = ros::Time::now();
  q_e_previous_ = q_e;

  // update proportional
  pid_msg_.proportional_term.x = q_e.x();
  pid_msg_.proportional_term.y = q_e.y();
  pid_msg_.proportional_term.z = q_e.z();
  // update integral
  pid_msg_.integrative_term.x = integral_term_.x;
  pid_msg_.integrative_term.y = integral_term_.y;
  pid_msg_.integrative_term.z = integral_term_.z;
  // update derivative
  pid_msg_.derivative_term.x = derivative_term_.x;
  pid_msg_.derivative_term.y = derivative_term_.y;
  pid_msg_.derivative_term.z = derivative_term_.z;
  pid_terms_pub_.publish(pid_msg_);
...
TorquesAndThrust RPGRotorsInterface::bodyRateControl(
  ...
  torques_and_thrust.body_torques =
      ...
      inertia_ * rate_cmd.angular_accelerations; // - 
      // torque_external_;   // subtract the reference for a P controller
  torques_and_thrust.collective_thrust = rate_cmd.collective_thrust;
  // std::cout << "external torque_x : " << torque_external_[0] << "\n";
...
void RPGRotorsInterface::loadParameters() {
  ...
  quadrotor_common::getParam("derivative_gain", derivative_gain_, 0.1, pnh_);
  quadrotor_common::getParam("integral_gain", integral_gain_, 3.0, pnh_);
  quadrotor_common::getParam("integral_threshold", integral_threshold_, 0.05, pnh_);
}

void RPGRotorsInterface::updateIntegralTerm(const Eigen::Quaterniond& q_e, const double& dt)
{
  // if (interaction_) 
  // {
  // x coordinate
  if(std::abs(integral_term_.x)<integral_threshold_ || integral_term_.x*q_e.x()*q_e.w() < 0) 
  {
    if (q_e.w()>0)
      integral_term_.x += integral_gain_ * q_e.x() * dt;
    else
      integral_term_.x -= integral_gain_ * q_e.x() * dt;
  }

    // y coordinate
  if(std::abs(integral_term_.y)<integral_threshold_ || integral_term_.y*q_e.y()*q_e.w() < 0) 
  {
    if (q_e.w()>0)
      integral_term_.y += integral_gain_ * q_e.y() * dt;
    else
      integral_term_.y -= integral_gain_ * q_e.y() * dt;
  }

  // z coordinate
  if(std::abs(integral_term_.z)<integral_threshold_ || integral_term_.z*q_e.z()*q_e.w() < 0) 
  {
    if (q_e.w()>0)
      integral_term_.z += integral_gain_ * q_e.z() * dt;
    else
      integral_term_.z -= integral_gain_ * q_e.z() * dt;
  }
  // }
...
void RPGRotorsInterface::updateDerivativeTerm(const Eigen::Quaterniond& q_e_previous, const Eigen::Quaterniond& q_e_current, const double& dt)
{
  if (q_e_current.w()>0 && q_e_previous.w()>0) 
  {
    derivative_term_.x = (q_e_current.x() - q_e_previous.x() ) / dt ;
    derivative_term_.y = (q_e_current.y() - q_e_previous.y() ) / dt ;
    derivative_term_.z = (q_e_current.z() - q_e_previous.z() ) / dt ;
  }
  else if (q_e_current.w()>0 && q_e_previous.w()<0)
  {
    derivative_term_.x = (q_e_current.x() + q_e_previous.x() ) / dt ;
    derivative_term_.y = (q_e_current.y() + q_e_previous.y() ) / dt ;
    derivative_term_.z = (q_e_current.z() + q_e_previous.z() ) / dt ;  
...
quadrotor_msgs::ControlCommand RPGRotorsInterface::toRosMsg(const TorquesAndThrust& torque_and_thrust_cmd)
{
  quadrotor_msgs::ControlCommand torque_and_thrust_msg;
  torque_and_thrust_msg.collective_thrust = torque_and_thrust_cmd.collective_thrust;
  torque_and_thrust_msg.torque = quadrotor_common::eigenToGeometry(torque_and_thrust_cmd.body_torques);

  return torque_and_thrust_msg;
}


void RPGRotorsInterface::InteractionCallback(
  const std_msgs::BoolConstPtr& msg) 
{
  interaction_ = msg->data;
}
