...
int ADCReader::getMaxAdcValue()
{
  switch (board_name_)
  {
    ...
    case BoardNames::KHADAS:
      return kMaxAdcValueKhadas_;
...
int ADCReader::setBoardName(const std::string& board_name)
{
  ...
  const std::string khadas_name = "khadas";
  ...
  else if (khadas_name.compare(board_name) == 0)
  {
    board_name_ = BoardNames::KHADAS;
  }
  else
  {
...
int ADCReader::adcConnect(const unsigned int adc_id)
{
  ...
  if (board_name_ == BoardNames::KHADAS && adc_id != 0 && adc_id != 3)
  {
    perror("On the Khadas Vim3, the ADC ID must be either 0 or 3!");
    return -1;
  }
